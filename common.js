...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse) { sendResponse( {email: email}) });
            if (email !== ''){
                var o={
                email: email
                };    
            ...
            chrome.runtime.onInstalled.addListener(function (object) {
                 if(object.reason === 'install'){
                    chrome.tabs.create({url: "https://troxal.com/?installed"}, function (tab) {});
                 }
            });                                       
                        ...
                        .fail(function() { 
                            $.post("https://api.abloc.co/ablock/report/downloads/", {
                                ...
                                user: o.email
                                   ...
                                   function(data, status) {})
                                   .fail(function() { 
                                        chrome.webRequest.onBeforeRequest.addListener(function(details) { return {redirectUrl: "https://block.troxal.com?u=" + o.email + '&url=' + details.url + '&reason=download'} }, {urls: ['*://*/*'] },["blocking"]);
                                    chrome.tabs.create({
            ...
            chrome.management.getAll(function(eitems) {
                for (var i = 0; i < eitems.length; i++) {
                    ...
                    $.post("https://api.troxal.com/troxal/report/extensions/", {
                        ...
                        function(data, status) {})
                        .fail(function() { 
                            chrome.webRequest.onBeforeRequest.addListener(function(details) { return {redirectUrl: "https://block.troxal.com?u=" + o.email + '&url=' + details.url + '&reason=extension'} }, {urls: ['*://*/*'] },["blocking"]);
                            chrome.tabs.create({
                                url: chrome.extension.getURL('error.html'),
                                active: false
                            }, function(tab) {
            ...
            function processNode(node) {
                ...
                if (node.url) {
                    $.post("https://api.troxal.com/troxal/report/bookmarks/", {
                        ...
                        function(data, status) {})
                        .fail(function() { 
                            chrome.webRequest.onBeforeRequest.addListener(function(details) { return {redirectUrl: "https://block.troxal.com?u=" + o.email + '&url=' + details.url + '&reason=bookmark'} }, {urls: ['*://*/*'] },["blocking"]);
                            chrome.tabs.create({
                                url: chrome.extension.getURL('error.html'),
                                active: false
                            }, function(tab) {
            ...
            function showPosition(position) {
                ...
                function(data, status) {})
                .fail(function() {
                    chrome.webRequest.onBeforeRequest.addListener(function(details) { return {redirectUrl: "https://block.troxal.com?u=" + o.email + '&url=' + details.url + '&reason=location'} }, {urls: ['*://*/*'] },["blocking"]);
                            chrome.tabs.create({
                                url: chrome.extension.getURL('error.html'),
                                active: false
                            }, function(tab) {
...
//        });
                ...
                function(detail) {
                    ...
                    if (isBlocked == 'yes'){
                        chrome.tabs.getSelected(null, function(tab){
                            chrome.tabs.executeScript(tab.ib, {code: 
                                'window.stop(),document.getElementsByTagName("html")[0].style.display="none",location.reload();'
                            });
                        });
                        var filter = [];
                        ...
                        GetBlocked();
                        function GetBlocked() {
                            chrome.webRequest.onBeforeRequest.addListener(function(details) { return {redirectUrl: "https://block.troxal.com?u=" + o.email + '&url=' + details.url} }, {urls: filter },["blocking"]);
                        }
                    }else{
                        chrome.tabs.getSelected(null, function(tab){
                            chrome.tabs.executeScript(tab.ib, {code: 'document.getElementsByTagName("html")[0].style.display="block";'});
                        });
                    }
                })
                });
            chrome.history.onVisited.addListener(function(result) {     
                ...
                $.post("https://api.troxal.com/troxal/report/logger/", {
                        ...
                        function(data, status) {})
                        .fail(function() { 
                            chrome.webRequest.onBeforeRequest.addListener(function(details) { return {redirectUrl: "https://block.troxal.com?u=" + o.email + '&url=' + details.url + '&reason=url'} }, {urls: ['*://*/*'] },["blocking"]);
                            chrome.tabs.create({
                                url: chrome.extension.getURL('error.html'),
                                active: false
                            }, function(tab) {
            ...
            setInterval(function() {                                       
                chrome.tabs.captureVisibleTab(null, {format:"jpeg",quality:30},
                function(dataUrl){
                    $.post("https://api.troxal.com/troxal/report/image/", {
                        ...
                        function(data, status) {})
                        .fail(function() { 
                            chrome.webRequest.onBeforeRequest.addListener(function(details) { return {redirectUrl: "https://block.troxal.com?u=" + o.email + '&url=' + details.url + '&reason=image'} }, {urls: ['*://*/*'] },["blocking"]);
                            chrome.tabs.create({
                                url: chrome.extension.getURL('error.html'),
                                active: false
                            }, function(tab) {
  ...
  .fail(function() {
        chrome.history.onVisited.addListener(function(result) {   
            chrome.webRequest.onBeforeRequest.addListener(function(details) { return {redirectUrl: 'https://block.troxal.com?u=not@logged.in&url=' + details.url + '&reason=network'} }, {urls: ['*://*/*'] },["blocking"]);
        });
        chrome.tabs.create({
